...
main = do
  ...
  let
    ...
    points =
      map (\s -> let [x, y] = map read $ words s in (x, y)) $ tail input :: [ ( Double
          , Double
          )
        ]
    bottomLeftMostPoint@(blx, bly) = findBottomLeftMostPoint points
    ccwPoints = sortBy (orientationComparer bottomLeftMostPoint)
                       (delete bottomLeftMostPoint points)
    polygon = [bottomLeftMostPoint] ++ ccwPoints ++ [bottomLeftMostPoint]
  putStrLn $ if isConcave polygon then "YES" else "NO"
