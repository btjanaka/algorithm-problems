...
addPoint vs@(p1:p2:ps) v =
  if calcOrientation p2 p1 v < 0
    then addPoint (p2 : ps) v
    else v : vs
...
calcHullLength points =
  let base = findBottomLeftMostPoint points
      polygon =
        [base] ++
        sortBy (orientationComparer base) (delete base points) ++ [base]
      hull = convexHull polygon
   in traceShow polygon $
      traceShow hull $ sum $ zipWith dist (init hull) (tail hull)
...
main = do
  ...
  let points =
        map (\[x, y] -> (x, y)) .
        map (map (read :: String -> Double)) . map words . lines $
        content
