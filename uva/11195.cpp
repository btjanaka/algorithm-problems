...
/*
// Old version - slower, TLE

// Store which rows, left diagonals, and right diagonals are occupied
bitset<30> rw, ld, rd;

void backtrack(int c) {  // c is the column that we are currently investigating
    if(c == n) {  // Found a solution
        ++res;
        return;
    }
    // Iterate through all the rows for this column and check if the
    // corresponding rows, left diagonals, and right diagonals are empty. Also
    // check if the cell is bad. The rw, ld, and rd tell us whether there is
    // already a queen in that line of the board.
    for(int r = 0; r < n; r++) {
